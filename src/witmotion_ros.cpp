...
std::vector<double> ROSWitmotionSensorController::imu_accel_covariance = {-1, 0, 0, 0, 0, 0, 0, 0, 0};
std::vector<double> ROSWitmotionSensorController::imu_velocity_covariance = {-1, 0, 0, 0, 0, 0, 0, 0, 0};
std::vector<double> ROSWitmotionSensorController::imu_orientation_covariance = {-1, 0, 0, 0, 0, 0, 0, 0, 0};
...
// rclcpp::Publisher<sensor_msgs::msg::NavSatFix>* ROSWitmotionSensorController::gps_publisher = nullptr;
// rclcpp::Publisher<geometry_msgs::msg::Twist>* ROSWitmotionSensorController::ground_speed_publisher = nullptr;
// rclcpp::Publisher<geometry_msgs::msg::Vector3>* ROSWitmotionSensorController::accuracy_publisher = nullptr;
                                                               ...
                                                               //Node("witmotion_ros")
{
    ...
    if (temp_enable)
    {
        ...
        else
        {
            ...
            RCLCPP_WARN_SKIPFIRST(rclcpp::get_logger("ROSWitmotionSensorController"), "Cannot determine message type to take temperature from (%s). Falling back to acceleration message", temp_from_str.c_str());
            temp_from = pidAcceleration;
        }
...
bool ROSWitmotionSensorController::Restart(std::shared_ptr<std_srvs::srv::Empty::Request> request, std::shared_ptr<std_srvs::srv::Empty::Response> response)
{
    RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "Attempting to restart sensor connection from SUSPENDED state");
    if (!suspended)
    {
        // ROS_WARN("Cannot restart:  the connection is not suspended");
        RCLCPP_WARN_STREAM_SKIPFIRST(rclcpp::get_logger("ROSWitmotionSensorController"), "Cannot restart:  the connection is not suspended" << 4);
        return false;
    }
    Instance().Start();
    suspended = false;
...
void ROSWitmotionSensorController::gps_process(const witmotion_datapacket &packet)
{
    if (gps_enable)
    {
        ...
        msg.position_covariance_type = have_accuracy ? sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_DIAGONAL_KNOWN : sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_UNKNOWN;
...
rclcpp::Node::SharedPtr ROSWitmotionSensorController::Start()
{
    ...
    if (rtc_enable && rtc_presync)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "Configuration ROM: lock removal started");
        ...
        emit ConfigureSensor(config_packet);
        sleep(1);
        RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "RTC pre-synchonization completed, saving configuration");
...
void ROSWitmotionSensorController::Packet(const witmotion_datapacket &packet)
{
    ...
    case pidGPSCoordinates:
        ...
        break;
    default:
        RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "Unknown packet ID 0x%X acquired", packet.id_byte);
    }
}

void ROSWitmotionSensorController::Error(const QString &description)
{
    RCLCPP_ERROR(rclcpp::get_logger("ROSWitmotionSensorController"),"Sensor error: %s", description.toStdString().c_str());
    RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "Entering SUSPENDED state");
    reader->Suspend();
    suspended = true;
}
